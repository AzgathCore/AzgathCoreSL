    ...
    static bool HandleLookupEventCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (uint32 id = 0; id < events.size(); ++id)
        {
            ...
            if (Utf8FitTo(descr, wNamePart))
            {
                ...
                char const* active = activeEvents.find(id) != activeEvents.end() ? handler->GetAzgathString(LANG_ACTIVE) : "";
            ...
            if (factionEntry)
            {
                ...
                if (locale < TOTAL_LOCALES)
                {
                    ...
                    if (factionState) // and then target != nullptr also
                    {
                        ...
                        std::string rankName = handler->GetAzgathString(index);
                        ...
                        ss << handler->GetAzgathString(LANG_FACTION_NOREPUTATION);
                            ...
                            if (target)
                            {
                                ...
                                switch (status)
                                {
                                    case QUEST_STATUS_COMPLETE:
                                        statusStr = handler->GetAzgathString(LANG_COMMAND_QUEST_COMPLETE);
                                        break;
                                    case QUEST_STATUS_INCOMPLETE:
                                        statusStr = handler->GetAzgathString(LANG_COMMAND_QUEST_ACTIVE);
                                        break;
                                    case QUEST_STATUS_REWARDED:
                                        statusStr = handler->GetAzgathString(LANG_COMMAND_QUEST_REWARDED);
                                        break;
                                    default:
                                        break;
                                }
        ...
        for (uint32 id = 0; id < sSkillLineStore.GetNumRows(); id++)
        {
            ...
            if (skillInfo)
            {
                ...
                if (locale < TOTAL_LOCALES)
                {
                    ...
                    {
                        knownStr = handler->GetAzgathString(LANG_KNOWN);
        ...
        for (SpellNameEntry const* spellName : sSpellNameStore)
        {
            if (SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellName->ID, DIFFICULTY_NONE))
            {
                ...
                if (locale < TOTAL_LOCALES)
                {
                    ...
                    if (rank)
                        ss << handler->GetAzgathString(LANG_SPELL_RANK) << rank;

                    if (handler->GetSession())
                        ss << "]|h|r";

                    if (talent)
                        ss << handler->GetAzgathString(LANG_TALENT);
                    if (passive)
                        ss << handler->GetAzgathString(LANG_PASSIVE);
                    if (learn)
                        ss << handler->GetAzgathString(LANG_LEARN);
                    if (known)
                        ss << handler->GetAzgathString(LANG_KNOWN);
                    if (active)
                        ss << handler->GetAzgathString(LANG_ACTIVE);
            ...
            if (CharTitlesEntry const* titleInfo = sCharTitlesStore.LookupEntry(id))
            {
                for (uint8 gender = GENDER_MALE; gender <= GENDER_FEMALE; ++gender)
                {
                    ...
                    if (locale < TOTAL_LOCALES)
                    {
                        ...
                        char const* knownStr = target && target->HasTitle(titleInfo) ? handler->GetAzgathString(LANG_KNOWN) : "";
            ...
            if (MapEntry const* mapInfo = sMapStore.LookupEntry(id))
            {
                ...
                if (locale < TOTAL_LOCALES)
                {
                    ...
                    if (mapInfo->IsContinent())
                        ss << handler->GetAzgathString(LANG_CONTINENT);

                    switch (mapInfo->InstanceType)
                    {
                        case MAP_INSTANCE:
                            ss << handler->GetAzgathString(LANG_INSTANCE);
                            break;
                        case MAP_RAID:
                            ss << handler->GetAzgathString(LANG_RAID);
                            break;
                        case MAP_BATTLEGROUND:
                            ss << handler->GetAzgathString(LANG_BATTLEGROUND);
                            break;
                        case MAP_ARENA:
                            ss << handler->GetAzgathString(LANG_ARENA);
                            break;
                    }
