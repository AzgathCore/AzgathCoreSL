...
public:
    ...
    static bool HandleModifyMoneyCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (moneyToAdd < 0)
        {
            ...
            TC_LOG_DEBUG("misc", handler->GetAzgathString(LANG_CURRENT_MONEY), std::to_string(targetMoney).c_str(), std::to_string(moneyToAdd).c_str(), std::to_string(newmoney).c_str());
            if (newmoney <= 0)
            {
        ...
        TC_LOG_DEBUG("misc", handler->GetAzgathString(LANG_NEW_MONEY), std::to_string(targetMoney).c_str(), std::to_string(moneyToAdd).c_str(), std::to_string(target->GetMoney()).c_str());
    ...
    static bool HandleModifyRepCommand(ChatHandler* handler, const char* args)
    {
        ...
        if ((amount == 0) && (rankTxt[0] != '-') && !isdigit(rankTxt[0]))
        {
            ...
            {
                std::string rank = handler->GetAzgathString(ReputationRankStrIndex[r]);
                if (rank.empty())
                    continue;
...
//demorph player or unit
    ...
    static bool HandleModifyCurrencyCommand(ChatHandler* handler, const char* args)
    {
        ...
        char* currencyId = strtok((char*)args, " ");
        const CurrencyTypesEntry* currencyType = sCurrencyTypesStore.LookupEntry(atoi(currencyId));
        if (!currencyType)
            return false;
        ...
        target->ModifyCurrency(atoi(currencyId), atoi(amount), true, true);
